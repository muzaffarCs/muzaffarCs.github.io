...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_photo SET photo_caption=?, photo_detail=? ,p_category_id=? WHERE photo_id=?");
			$statement->execute(array($_POST['photo_caption'],$_POST['photo_detail'],$_POST['p_category_id'],$_REQUEST['id']));
    	} else {
        	...
        	// updating into the database
			$statement = $pdo->prepare("UPDATE tbl_photo SET photo_caption=?,photo_detail=? , photo_name=?, p_category_id=? WHERE photo_id=?");
			$statement->execute(array($_POST['photo_caption'],$_POST['photo_detail'],$final_name,$_POST['p_category_id'],$_REQUEST['id']));
    	}
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Photo Detail <span>*</span></label>
							<div class="col-sm-4">
								<input type="text" class="form-control" name="photo_detail" value="<?php echo $photo_detail ?>">
							</div>
						</div>
						<div class="form-group">
